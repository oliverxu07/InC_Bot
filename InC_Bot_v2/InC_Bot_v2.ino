...
void autopilotToggle() {
  ...
  if (interruptTime - lastAutopilotInterruptTime > 200) {
    autopilotActive = (autopilotActive == 1) ? 0 : 1;
  }
  lastAutopilotInterruptTime = interruptTime;
}

void muteToggle() {
  unsigned long interruptTime = millis();
  if (interruptTime - lastMuteInterruptTime > 200) {
    muteActive = (muteActive == 1) ? 0 : 1;
  }
  lastMuteInterruptTime = interruptTime;  
}
...
void setup() {
  ...
  pinMode(B3_mute, INPUT);  // button to mute tones and LEDs
  attachInterrupt(digitalPinToInterrupt(B3_mute), muteToggle, RISING);
